...
G_DEFINE_TYPE_WITH_CODE (GstEmbeddingOverlay, gst_embedding_overlay,
    ...
    GST_DEBUG_CATEGORY_INIT (gst_embedding_overlay_debug_category,
        "embeddingoverlay", 0,
        "debug category for embedding_overlay element"));
...
static void
gst_embedding_overlay_init (GstEmbeddingOverlay *
    embedding_overlay)
{
}
...
if (diff < 1.0){
  str = cv::format ("Pass");
  /* Get size of string on screen */
  size = cv::getTextSize (str, cv::FONT_HERSHEY_TRIPLEX, thickness, font_scale, &baseline);
  /* Put string on screen */
  cv::putText (cv_mat, str, cv::Point (0, size.width), cv::FONT_HERSHEY_PLAIN, font_scale, forest_green, thickness);
  /* Draw rectangle */
  cv::rectangle (cv_mat, cv::Point (0,0), cv::Point (width, height), forest_green, thickness);
} else {
  str = cv::format ("Fail");
  /* Get size of string on screen */
  size = cv::getTextSize (str, cv::FONT_HERSHEY_TRIPLEX, thickness, font_scale, &baseline);
  /* Put string on screen */
  cv::putText (cv_mat, str, cv::Point (0, size.width), cv::FONT_HERSHEY_PLAIN, font_scale, chilli_red, thickness);
  /* Draw rectangle */
  cv::rectangle (cv_mat, cv::Point (0,0), cv::Point (width, height), chilli_red, thickness);
}
